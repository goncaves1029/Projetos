...
define('EDIT_PASS', 'senhaEditar123'); // ALTERE para sua senha
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_edit') {
    ...
    if ($password !== EDIT_PASS) {
        ...
        $sql = "UPDATE presencas SET culto=:culto, data=:data, hora=:hora,
                ...
                link=:link, louvor=:louvor, comunicacao=:comunicacao, voluntarios=:voluntarios, pastores=:pastores,
...
<?php else: ?>
    <form method="post" action="edit.php?id=<?= (int)$id ?>">
        ...
        <label>Pastores (texto ou nomes separados por vírgula): <input type="text" name="pastores" value="<?= htmlspecialchars($row['pastores']) ?>"></label>
